...
async def check_new_posts(app: Application):
    """Проверяет таблицу на новые посты со статусом 'pending'"""
    while True:
        try:
            ws = get_sheet()
            ...
            for i, row in enumerate(rows, start=2):
                if row.get("Статус") == "pending" and row.get("Текст поста"):
                    ...
                    logger.info(f"Отправлен пост {post_id} на подтверждение")
    ...
    try:
        ...
        if action == "pub":
            ...
            await query.edit_message_text(
                text=f"✅ *Опубликовано в {CHANNEL_ID}*\n\n{post_text[:400]}...",
                parse_mode="Markdown"
            )
            logger.info(f"Пост из строки {row} опубликован")
            ...
            await query.edit_message_text(
                text=f"✏️ *Пост отправлен на редактирование*\n\nОткрой таблицу, отредактируй текст и поставь статус *pending* снова.\n\n[Открыть таблицу](https://docs.google.com/spreadsheets/d/{SHEETS_ID})",
                parse_mode="Markdown"
            )
